...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'amruser' || config.AFPLK == 'anuser' || config.AFPLK == 'vava' || config.AFPLK == 'Amruser') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'amruser' || config.AFPLK !== 'anuser' || config.AFPLK !== 'vava' || config.AFPLK !== 'Amruser') {
         console.log(
